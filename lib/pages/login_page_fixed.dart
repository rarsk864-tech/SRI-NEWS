...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _createUserAccount() async {
    if (role != LoginRole.user) {
      _message('Create Account is available only for User.');
      return;
    }
    ...
    if (applicantName.isEmpty || e.isEmpty || p.isEmpty || cp.isEmpty) {
      _message('Name, email, password and confirm password are required.');
      return;
    }
    ...
    try {
      ...
      // AuthService.register() already creates /users/{uid}.
      // Avoid a second write here because it would be an UPDATE.

      await FirebaseAuth.instance.signOut();

      if (!mounted) return;
      _message('Account created successfully. Please login.');
      setState(() {
      ...
      child: OutlinedButton.icon(
        onPressed: loading
            ...
            : () => setState(() {
                  role = value;
                  isCreateAccount = false;
                  if (value != LoginRole.admin && value != LoginRole.reporter) {
                    isApply = false;
                  }
                }),
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (role != LoginRole.user &&
                          !applyAllowed &&
                          (isApply || isCreateAccount))
                        const Padding(
                          padding: EdgeInsets.only(bottom: 12),
                          child: Text(
                            'This option is available only for User, Admin and Reporter.',
                            textAlign: TextAlign.center,
                            style: TextStyle(color: Colors.red),
                          ),
                        ),
                      if (isApply || isCreateAccount) ...[
