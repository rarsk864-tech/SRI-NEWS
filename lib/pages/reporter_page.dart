  ...
  Future<void> submitPost() async {
    ...
    try {
      ...
      if (selectedImages.length > maxImages) {
        throw Exception('Maximum $maxImages images can be selected.');
      }
      ...
      // Keep the reporter post safely below Firestore's document-size limit.
      final totalImageDataSize =
          mediaUrls.fold<int>(0, (sum, url) => sum + url.length);
      if (totalImageDataSize > 850000) {
        throw Exception(
          'Selected images are too large together. Please select fewer or smaller images.',
        );
      }
              ...
              OutlinedButton.icon(
                onPressed: loading ? null : () async {
                  ...
                  if (picked.length > 3 && mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Maximum 3 images can be selected.'),
                      ),
                    );
                  }
                },
