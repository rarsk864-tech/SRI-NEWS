...
class _OwnerPageState extends State<OwnerPage> {
  ...
  Future<void> _approvePost(
    ...
    try {
      ...
      String imageUrl = (p['mediaUrl'] ?? '').toString();
      final mediaPath = (p['mediaPath'] ?? '').toString();
      if (mediaPath.isNotEmpty) {
        try {
          final mediaRef = FirebaseStorage.instance.ref(mediaPath);
          await mediaRef.getMetadata();
          imageUrl = await mediaRef.getDownloadURL();
        } catch (e) {
          throw Exception('Reporter image upload is missing. Please ask the reporter to re-submit the post with the image.');
        }
      }
