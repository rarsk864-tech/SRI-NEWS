...
class _OwnerPageState extends State<OwnerPage> {
  ...
  Future<String> _uploadNewsImage(File image, String uid) async {
    final fileName = '${DateTime.now().millisecondsSinceEpoch}_${image.path.split('/').last.replaceAll(RegExp(r'[^A-Za-z0-9._-]'), '_')}';
    final ref = FirebaseStorage.instance.ref().child('news_images').child(uid).child(fileName);
    ...
    throw lastError ?? Exception('Unable to get uploaded image URL.');
  }
            ...
            Future<void> publish() async {
              ...
              try {
                ...
                if (image != null && uid.isNotEmpty) {
                  imageUrl = await _uploadNewsImage(image, uid);
                  imagePath = ''; // imageUrl is sufficient for published owner posts.
                }
  ...
  Future<void> _approvePost(
    ...
    try {
      ...
      final mediaPath = (p['mediaPath'] ?? '').toString();
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (_, snapshot) {
        ...
        for (final doc in docs) {
          final data = doc.data();
          final role = (data['role'] ?? '').toString().trim().toLowerCase();
          final name = (data['name'] ?? data['displayName'] ?? data['email'] ?? 'Unnamed User').toString().trim();
        ...
        Widget people(List<Map<String, dynamic>> list, String emptyText, {bool canManageAdmin = false, bool canManageReporter = false}) {
          if (list.isEmpty) {
            return Padding(
          ...
          return Column(
            children: list.map((person) {
              final isAdmin = person['role'] == 'admin' || person['role'] == 'administrator';
              return ListTile(
                dense: true,
                ...
                title: Text(person['name'] as String, style: const TextStyle(fontWeight: FontWeight.w700)),
                subtitle: Text(
                  '${person['email'] as String}${isAdmin ? ' • ADMIN' : ''}',
                ),
                trailing: canManageAdmin
                    ? Wrap(
                        spacing: 6,
                        children: [
                          OutlinedButton.icon(
                          ...
                          if (canManageReporter)
                            OutlinedButton.icon(
                              ...
                              label: const Text('Make Reporter'),
                            ),
                        ],
                      )
                    : isAdmin
          ...
          child: Column(
            children: [
              ExpansionTile(
                ...
                title: Text('Users (${users.length})', style: const TextStyle(fontWeight: FontWeight.w800)),
                children: [people(users, 'No users found.', canManageAdmin: true, canManageReporter: true)],
              ),
              const Divider(height: 1),
              ExpansionTile(
                leading: const Icon(Icons.edit_note_outlined),
                title: Text('Reporters (${reporters.length})', style: const TextStyle(fontWeight: FontWeight.w800)),
                children: [people(reporters, 'No reporters found.', canManageAdmin: true)],
              ),
              const Divider(height: 1),
              ExpansionTile(
                leading: const Icon(Icons.admin_panel_settings_outlined),
                title: Text('Admins (${admins.length})', style: const TextStyle(fontWeight: FontWeight.w800)),
                children: [people(admins, 'No admins found.')],
              ),
              const Divider(height: 1),
              ExpansionTile(
                leading: const Icon(Icons.verified_user_outlined),
                title: Text('Owners (${owners.length})', style: const TextStyle(fontWeight: FontWeight.w800)),
                children: [people(owners, 'No owners found.')],
              ),
            ],
          ),
        );
