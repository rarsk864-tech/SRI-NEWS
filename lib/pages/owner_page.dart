...
class _OwnerPageState extends State<OwnerPage> {
  ...
  Future<String?> _pickAndUploadNewsImage() async {
    final picked = await _picker.pickImage(source: ImageSource.gallery, imageQuality: 88);
    if (picked == null) return null;
    final uid = FirebaseAuth.instance.currentUser?.uid ?? 'owner';
    final ref = FirebaseStorage.instance.ref().child('news_images/${uid}_${DateTime.now().millisecondsSinceEpoch}.jpg');
                    ...
                    children: [
                      ...
                      OutlinedButton.icon(
                        onPressed: saving ? null : () async {
                          final url = await _pickAndUploadNewsImage();
                          if (url != null) setSheetState(() => imageUrl = url);
                        },
  ...
  Future<void> _changeUserRole({
    ...
    try {
      await db.collection('users').doc(uid).set({
        'role': newRole,
        'roleLabel': newRole == 'admin' ? 'ADMIN' : 'USER',
