...
class _NewsCardState extends State<NewsCard> {
  ...
  String _dateLabel(DateTime? date, String fallback) {
    if (date == null) return fallback.isEmpty ? 'Today' : fallback;
  ...
  Widget _newsVisual() {
    ...
    return SizedBox(
      ...
      child: Stack(
        fit: StackFit.expand,
        children: [
          Image.network(image, fit: BoxFit.cover, errorBuilder: (_, __, ___) => _fallbackImage()),
          if (item.breaking)
            Positioned(
              top: 18, right: 20,
              child: Container(
                ...
                child: const Text('BREAKING', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w900)),
              ),
            ),
          Positioned(
            left: 16, bottom: 12,
            child: Container(
              ...
              child: Text(_dateLabel(item.publishedAt, item.time), style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w700)),
            ),
          ),
        ],
      ),
        ...
        children: [
          ...
          Positioned(
            ...
            child: Container(
              ...
              child: Text(
                _dateLabel(item.publishedAt, item.time),
...
class AccountPage extends StatelessWidget {
  ...
  Future<void> _openAdminLogin(BuildContext context) async {
    ...
    if (email.text.trim().isEmpty || password.text.isEmpty) {
      ...
      password.dispose();
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Admin email and password are required.')),
      );
      return;
    }
    ...
    try {
      ...
      if (uid == null) throw Exception('Admin account not found.');
      ...
      if (context.mounted) {
        await _openAdminDashboard(context);
      }
          ...
          return ListView(
            ...
            children: [
              ...
              _accountAction(
                ...
                title: 'Admin Login',
                subtitle: 'Admin dashboard, post approval and upload',
                onTap: () => _openAdminLogin(context),
              ),
              const SizedBox(height: 10),
              _accountAction(
              ...
              children: [
                ...
                if (!isOwner && !isAdmin)
                  _accountAction(
                    icon: Icons.verified_user_outlined,
                    title: 'Admin Login',
                    subtitle: 'Sign in with the Admin account',
                    onTap: () => _openAdminLogin(context),
                  ),

                if (!isOwner && !isAdmin) const SizedBox(height: 10),
