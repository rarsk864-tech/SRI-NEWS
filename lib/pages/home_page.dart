...
class _BreakingTickerState extends State<_BreakingTicker> {
  ...
  Future<void> _autoScroll() async {
    while (mounted && _running && _controller.hasClients) {
      final max = _controller.position.maxScrollExtent;
      if (max <= 0) break;
      await _controller.animateTo(
        max,
        duration: Duration(milliseconds: 14000 + widget.items.length * 900),
        curve: Curves.linear,
      );
  ...
  @override
  void dispose() { _running = false; _controller.dispose(); super.dispose(); }
  @override
  Widget build(BuildContext context) {
    return SizedBox(
      height: 48,
            ...
            child: InkWell(
              ...
              child: Container(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  const Text('BREAKING', style: TextStyle(color: _blue, fontWeight: FontWeight.w900)),
                  const SizedBox(width: 5),
                  const Text('NEWS', style: TextStyle(color: _red, fontWeight: FontWeight.w900)),
                  const SizedBox(width: 8),
                  Text(text, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: Color(item.titleColor), fontWeight: FontWeight.w800)),
                ]),
              ),
            ),
          );
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (matter.isNotEmpty)
                      Text(matter, style: TextStyle(color: Color(item.matterColor), fontSize: 18, height: 1.65)),
                  ],
                ),
              ),
            ),
