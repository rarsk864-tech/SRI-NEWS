        ...
        return StatefulBuilder(builder: (context, setSheetState) {
          Future<void> submit() async {
            ...
            try {
              ...
              final mediaUrls = <String>[];
              for (final image in detailImages.take(5)) {
                final bytes = await image.readAsBytes();
                final uploaded = await StorageUploadService.uploadCarouselJpeg(folder: 'reporter_posts', uid: user.uid, bytes: bytes);
                mediaUrls.add(uploaded.dataUrl);
              }
          ...
          return SafeArea(
            child: Padding(
              ...
              child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                ...
                TextField(controller: tagController, enabled: !saving, decoration: const InputDecoration(labelText: 'Tag (optional)', border: OutlineInputBorder())),
                ...
                OutlinedButton.icon(
                  onPressed: saving ? null : () async {
                    ...
                    if (picked.length > 5 && context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Maximum 5 images can be selected.')));
                  },
                  icon: const Icon(Icons.collections_outlined),
                  label: Text(detailImages.isEmpty ? 'Select up to 5 Images' : '${detailImages.length} Images Selected'),
                ),
