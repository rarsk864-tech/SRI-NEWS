...
class _OwnerPageState extends State<OwnerPage> {
  ...
  Future<void> _editNews(
    ...
    String currentImageUrl = (data['imageUrl'] ?? '').toString();
    File? selectedImage;
  ...
  Future<void> _removeReporter({
    ...
    try {
      ...
      // Remove Reporter role and approval state from the existing user document.
      // Keep the user account itself intact.
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (_, snapshot) {
        ...
        for (final doc in docs) {
          ...
          final roleLabel =
              (data['roleLabel'] ?? '').toString().trim().toLowerCase();
          final reporterApproved =
              (data['reporterStatus'] ?? '').toString().trim().toLowerCase() ==
                  'approved';

          // Support both the current role field and older roleLabel data.
          final effectiveRole = role == 'owner' || roleLabel == 'owner'
              ? 'owner'
              : (role == 'admin' ||
                      role == 'administrator' ||
                      roleLabel == 'admin' ||
          ...
          if (effectiveRole == 'owner') {
            ...
            admins.add(person);
              ...
              (data['reporterStatus'] ?? '').toString().toLowerCase() ==
                  'approved') {
            ...
            children: list.map((person) {
                  ...
                  trailing = Wrap(
                    ...
                    children: [
                      ...
                      OutlinedButton.icon(
                        ...
                        label: const Text('Remove Reporter'),
                      ),
                    ],
                  );
                } else if (isAdmin) {
              ...
              return ListTile(
                ...
                leading: CircleAvatar(
                  radius: 18,
                  backgroundColor: isOwner ? Colors.blueGrey : _ownerRed,
                  child: Icon(
                    isOwner
                ...
                subtitle: Text(
                  '${person['email'] as String} • $roleLabel',
                ),
                trailing: trailing,
              );
            }).toList(),
