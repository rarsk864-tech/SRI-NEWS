...
class StorageUploadService {
  ...
  static Future<StorageUploadResult> uploadCarouselJpeg({
    ...
    Uint8List jpeg = Uint8List.fromList(img.encodeJpg(resized, quality: 38));

    if (jpeg.length > 30 * 1024) {
      resized = img.copyResize(resized, width: 560);
      jpeg = Uint8List.fromList(img.encodeJpg(resized, quality: 34));
    }
    if (jpeg.length > 30 * 1024) {
    ...
    if (jpeg.length > 30 * 1024) {
      ...
      jpeg = Uint8List.fromList(img.encodeJpg(resized, quality: 27));
    }
    if (jpeg.length > 30 * 1024) {
      resized = img.copyResize(resized, width: 320);
      jpeg = Uint8List.fromList(img.encodeJpg(resized, quality: 24));
    }
    if (jpeg.length > 30 * 1024) {
      resized = img.copyResize(resized, width: 280);
      jpeg = Uint8List.fromList(img.encodeJpg(resized, quality: 21));
    }
    if (jpeg.length > 30 * 1024) {
      resized = img.copyResize(resized, width: 240);
      jpeg = Uint8List.fromList(img.encodeJpg(resized, quality: 18));
    }
    if (jpeg.length > 30 * 1024) {
