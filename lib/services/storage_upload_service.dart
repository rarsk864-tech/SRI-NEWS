...
/// Firebase Storage-free image handling.
/// Images are resized/compressed and stored as data URLs
/// in the existing Firestore document.
...
class StorageUploadService {
  static Future<StorageUploadResult> uploadJpeg({
    ...
    // Keep good resolution for single-image news posts.
    ...
    // Firestore document limit is about 1 MiB. Keep the encoded image
    // safely below that limit while avoiding unnecessary quality loss.
    Uint8List jpeg =
        Uint8List.fromList(img.encodeJpg(resized, quality: 62));

    const maxJpegBytes = 620 * 1024;

    if (jpeg.length > maxJpegBytes) {
      resized = img.copyResize(resized, width: 1120);
      jpeg = Uint8List.fromList(img.encodeJpg(resized, quality: 58));
    }

    if (jpeg.length > maxJpegBytes) {
      resized = img.copyResize(resized, width: 1024);
      jpeg = Uint8List.fromList(img.encodeJpg(resized, quality: 54));
    }

    if (jpeg.length > maxJpegBytes) {
      resized = img.copyResize(resized, width: 900);
      jpeg = Uint8List.fromList(img.encodeJpg(resized, quality: 50));
    }

    if (jpeg.length > maxJpegBytes) {
      resized = img.copyResize(resized, width: 800);
      jpeg = Uint8List.fromList(img.encodeJpg(resized, quality: 46));
    }

    if (jpeg.length > maxJpegBytes) {
      resized = img.copyResize(resized, width: 720);
    ...
    if (jpeg.length > maxJpegBytes) {
      throw Exception(
        'Image is too large after compression. Please choose another image.',
      );
    }
  ...
  static Future<StorageUploadResult> uploadCarouselJpeg({
    ...
    // Total image data is kept below the Firestore document limit.
    // More images = smaller per-image budget, while preserving quality
    // as much as possible for text-heavy news images.
    ...
    for (final maxDimension in maxDimensions) {
      ...
      if (resized.width > maxDimension ||
          resized.height > maxDimension) {
        resized = img.copyResize(
          resized,
